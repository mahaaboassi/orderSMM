...
const PeriodsWithControl = () => {
  ...
  const deleteData = async()=>{
        ...
        const { response, message, statusCode} = await Helper({
            url : apiRoutes.payment_periods.delete(currentData.id),
            method : "DELETE",
            hasToken : true
        })
        if(response){
  ...
  return (
    <div className="settings flex flex-col gap-4">
      {loading ? (
        ...
        <>
         <div className="flex gap-2 items-center">
                ...
                <Link to={"/dashboard/admin/settings/periods/add"}>Add new</Link>
            </div>
          ...
          <div className="flex flex-col gap-4">
            {data.map((e)=>(<div className="grid grid-cols-1 sm:grid-cols-4 gap-3 card p-5">
                  <div className="flex items-center">
                    <strong>Name:</strong> &nbsp; {e.name}
                  </div>

                  <div className="flex items-center">
                    <strong>Factor:</strong> &nbsp; {e.factor}
                  </div>
                  ...
                  <div className="flex gap-2 items-end">
                    <button onClick={()=>navigate(`/dashboard/admin/settings/periods/add/${e.id}`)} className="dark-btn !text-sm">Edit</button>
                    <button
                      type="button"
      ...
      {
        "id" in currentData && <PopupCalled  open={"id" in currentData ? true : false} close={()=>{setCurrentData({})}} children={<div className="flex flex-col gap-5">
