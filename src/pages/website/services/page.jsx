...
const Services = ()=>{
    ...
    const getAds = async (signal) => {
        ...
        if(response){
            ...
            setAds(response.data.sort(() => 0.5 - Math.random()).slice(0, 5))
            setLoadingAds(false)
        }else{
            console.log(message);
    ...
    return(<div className="px-2 lg:px-16 flex flex-col gap-5 ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2  lg:grid-cols-4 gap-4">
            {
               loading ? [...Array(4)].map((_,i)=>(<div  className="h-20 w-full rounded-xl bg-gray-300 animate-pulse" key={`Skeleton_Pin_Up_${i}`} >
                </div>)): services?.pinnedUp && services.pinnedUp.length > 0 && 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {
               loading ? [...Array(4)].map((_,i)=>(<div  className="h-20 w-full rounded-xl bg-gray-300 animate-pulse" key={`Skeleton_Pin_Down_${i}`} >
                </div>)): services?.pinnedDown && services.pinnedDown.length > 0 && 
