...
const Notifications = ()=>{
    ...
    useEffect(()=>{
        ...
        if(isCallRedux) {
            getData(signal)
            dispatch(callStatus({isCall : false}))
        }
        return () => controller.abort()
    },[isCallRedux])
    const getData = async(signal, page = 1)=>{
        setLoading(true)
        const params = {
            page : 1
        }
        const { response , message, statusCode } = await Helper({
            url : apiRoutes.notifications.byUser,
            signal : signal,
            method : "GET",
            params: params,
            hasToken : true
        ...
        if(response){
            if(response.data.length >0 && finalObj?.id != response.data[0].id && "id" in finalObj) setIsNew(true)
            setData(response.data)
            setLoading(false)
            setFinalObj(prev=> response.data.length>0?response.data[0]:{})
    ...
    return(<div ref={menuRef} className="relative flag-container">
        <div onClick={()=>{
            setIsOpen(!isOpen)
            setIsNew(false)
        }} className="cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 14 15" fill="none">
                <g clipPath="url(#clip0_355_17)">
                <path fillRule="evenodd" clipRule="evenodd" d="M7.90975 0.840749C10.0136 1.28668 11.6104 3.25253 11.6104 5.59193C11.6104 8.35003 11.3613 10.4242 14 12.2712C9.32598 12.2712 4.67378 12.2712 0 12.2712C2.64587 10.419 2.39602 8.3051 2.39602 5.59193C2.39602 3.23996 4.01023 1.2658 6.13064 0.833669C6.25295 -0.282072 7.79533 -0.276701 7.90975 0.840749ZM8.90206 13.3289C8.73983 14.2792 7.9499 14.9998 6.99983 14.9998C6.04975 14.9998 5.26005 14.2793 5.09747 13.3289H8.90206Z" fill="white"/>
                </g>
                <defs>
        ...
        { isNew && <div className={`new ${(i18n.language == "ar" || i18n.language == "ur")?"-left-0.5":"-right-0.5"}  absolute`} ></div>}
        {isOpen && <div className={`${data.length > 0 ? "notification-menu" : "notification-menu-min"} p-3 flex flex-col gap-2 ${(i18n.language == "ar" || i18n.language == "ur")?" -left-20 xs:left-0":"-right-20 xs:right-0"}`}>
            {loading? [...Array(7)].map((_,i)=>(<div  className="h-8 w-full rounded-xl bg-gray-300 animate-pulse" key={`Skeleton_Ads_${i}`} >
                </div>)): <>
                { data.length > 0 ? <>
                {
                    ...
                    <Link to={`/dashboard/${user?.role == "admin"? "admin/notifications" : "notifications" }`}>  
                        <div onClick={()=>setIsOpen(false)} className="flex gap-2 items-center">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="12" height="13" viewBox="0 0 52 53" fill="none">
                                ...
                                <path d="M51.4752 25.862L1.17665 0.725559C1.08057 0.677406 0.976011 0.648939 0.86899 0.641799C0.761969 0.63466 0.654607 0.648989 0.553095 0.68396C0.451583 0.718931 0.35793 0.773854 0.277535 0.845559C0.19714 0.917265 0.131593 1.00434 0.0846756 1.10175C0.0242504 1.21775 -0.00732422 1.34686 -0.00732422 1.47794C-0.00732422 1.60902 0.0242504 1.73813 0.0846756 1.85414L13.1503 26.6486L0.097373 51.426C0.0369478 51.542 0.00537316 51.6711 0.00537316 51.8022C0.00537316 51.9333 0.0369478 52.0624 0.097373 52.1784C0.144291 52.2758 0.209837 52.3629 0.290232 52.4346C0.370627 52.5063 0.464281 52.5612 0.565793 52.5962C0.667304 52.6311 0.774666 52.6455 0.881687 52.6383C0.988709 52.6312 1.09327 52.6027 1.18935 52.5546L51.4879 27.4181C51.6357 27.3501 51.761 27.2406 51.8488 27.1026C51.9367 26.9647 51.9834 26.8041 51.9834 26.6401C51.9834 26.476 51.9367 26.3155 51.8488 26.1775C51.761 26.0395 51.6357 25.93 51.4879 25.862H51.4752Z" fill="#08392B"/>
                                </g>
                                <defs>
                                <clipPath id="clip0_363_51">
                                <rect width="52" height="52" fill="white" transform="translate(0 0.640015)"/>
                                </clipPath>
                                </defs>
                            </svg>
                        </div>
